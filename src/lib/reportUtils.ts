...
export const exportToExcel = (patients: PatientEntry[], report: HealthReport) => {
  ...
  const sqlSchema = [
    ...
    [");"],
    [""],
    ["-- 3. POLÍTICAS DE SEGURIDAD (RLS) - Opcional para desarrollo"],
    ["ALTER TABLE pacientes ENABLE ROW LEVEL SECURITY;"],
    ["CREATE POLICY \"Permitir todo a usuarios autenticados\" ON pacientes FOR ALL USING (true);"],
    ["ALTER TABLE reportes_diarios ENABLE ROW LEVEL SECURITY;"],
    ["CREATE POLICY \"Permitir todo a usuarios autenticados\" ON reportes_diarios FOR ALL USING (true);"]
  ];
